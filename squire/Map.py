...
def get_header(bamfile,headerfile):
    samtoolscommand_list = ["samtools","view","-H", "-@", str(pthreads), bamfile, ">",headerfile]
...
def mask_reads(infile,extra,chrom_list,basename,outfolder,pthreads,debug):
    ...
    for chrom in chrom_list:
        search="""'"""+ '$3 ~ /' + chrom + '/' + """'"""
        dupe_command_list = ["samtools","view","-@", str(pthreads), infile,chrom, ">", ectopic_alignments] #skips lines if the read has already appeared in the file
        dupe_command = " ".join(dupe_command_list)
        sp.check_call(["/bin/bash", "-c", dupe_command])

        awkcommand_list = ["samtools","view", "-@", str(pthreads), infile,  ">",sam_temp] #writes lines in combined_tempfile that are not in unique_tempfile2 -> duplicates
